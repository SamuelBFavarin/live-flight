...
- The response must to at least include: flight identification, airline, aircraft model, origin, destination, current speed, the aircraft's current latitude and longitude (so the client can plot it on a map) and the aircraft's true track (heading in decimal degrees clockwise from north) so the client can rotate the aircraft icon. When OpenSky does not provide the heading, the field must to be `null`
