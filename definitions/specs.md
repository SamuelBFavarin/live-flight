...
- When the departure airport has known coordinates, the UI must to draw a line on the map between the departure airport and the aircraft's current position, giving a visual trail of how far the aircraft has travelled so far. The line must to update on every refresh and every dead-reckon frame (endpoint follows the plane marker); it must to be hidden when no flight is returned or when the departure coordinates are unknown.
