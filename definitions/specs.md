...
- `GET /aircraft-photo?icao24=<hex>` — returns a photo of the aircraft with the given ICAO24 address, sourced from a public aircraft photo registry (e.g. `planespotters.net`). The response must to include a thumbnail URL, the photographer's name and a link back to the photo page so callers can comply with the registry's attribution requirements.
  ...
  - `500` — upstream error.
- `GET /flight-track?icao24=<hex>` — returns the aircraft's recent trajectory as reported by OpenSky's experimental `/tracks/all` endpoint (`get_track_by_aircraft`). The response body is `{"track": {...}}` with `icao24`, `callsign`, `start_time`, `end_time` and a list of `path` waypoints (each with `time`, `latitude`, `longitude`, `baro_altitude`, `true_track`, `on_ground`).
  - `200` — success. When OpenSky has no track for this aircraft (endpoint is experimental and often returns nothing), return `{"track": null}` (still 200). Upstream network errors must also resolve to `{"track": null}` rather than propagating as 5xx so the UI can degrade gracefully.
  - `400` — `icao24` is missing or is not a valid 6-character hex string.
  - `429` — the caller's IP has exceeded the per-IP rate limit (see below).
...
- The UI must to draw a visual trail showing where the aircraft has been. The trail combines two sources:
  1. **Observed trajectory** — the aircraft's real path pulled from `GET /flight-track?icao24=<hex>` on every refresh when a flight is present. The UI must to render a solid polyline through each reported waypoint, ending at the aircraft's current position. The trailing endpoint must to follow the plane marker on every dead-reckon frame so the trail stays glued to the moving aircraft.
  2. **Estimated connector** — when the departure airport has known coordinates, a dashed segment from the departure airport to the first observed waypoint (or directly to the aircraft's current position if OpenSky returned no waypoints). The dashed styling must to visually distinguish the estimated portion from the observed portion.
  When OpenSky returns no track and the departure airport is unknown, no trail is drawn. The trail must to be hidden whenever no flight is returned.
...
- Origin and destination must to include the airport ICAO code, airport name, city/region, country code and (when available) the airport's latitude/longitude; these details must to be resolved from a public airport registry (e.g. `hexdb.io`). When the registry does not return a match, show `N/A` for the string fields and `null` for latitude/longitude


### Security & Abuse Prevention

- The `/closest-flight` and `/flight-track` endpoints must to each apply a per-IP rate limit of **10 requests per minute** (separate buckets per endpoint). Requests above that threshold must to return `429` without invoking the OpenSky client, to prevent DDoS-style abuse and to protect the configured OpenSky token from being exhausted.
